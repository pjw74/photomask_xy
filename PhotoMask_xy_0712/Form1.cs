...
namespace PhotoMask_xy_0712
{
    public partial class Form1 : Form
    {
        ...
        string serialNo1 = "68250369";
        string serialNo2 = "68250412";
        string serialNo3 = "68250413";
        ...
        private void stg_con_Click(object sender, EventArgs e)
        {
            ...
            {
                textBox1.Text = ("XY Stage Connected");
        ...
        private void setup_point_Click(object sender, EventArgs e)
        {
            decimal speed_1 = spd_value.Value;
            string spd = speed_1.ToString(spd_str);
            ...
            var comm_start_1 = move_axis_xy("0", x_stage_point, y_stage_point);
            send_function(comm_start_1);

            x_start = x_stage_point;
            y_start = y_stage_point;
            //z_start = z_stage_point;

            //double x_setup_db = CustomRound(RoundType.Truncate, x_stage_point, 1);
            //double y_setup_db = CustomRound(RoundType.Truncate, y_stage_point, 1);
            //double z_setup_db = CustomRound(RoundType.Truncate, z_stage_point, 1);
            ...
            var task_setup = Task.Run(() =>
            {
                while (true)
                {
                    ...
                    if (x_complete == Convert.ToDecimal(x_stage_point) && y_complete == Convert.ToDecimal(y_stage_point))
                    //&& z_complete == Convert.ToDecimal(z_stage_point)) // y축 좌표 비교 이동 완료
                    {
        ...
        public byte[] move_axis_xy(string channel, double x_axis_point, double y_axis_point)
        {
            byte[] command = Encoding.UTF8.GetBytes("BC");
            byte[] channel_ba = Encoding.UTF8.GetBytes(channel);
            byte[] motion_type = Encoding.UTF8.GetBytes("0");
        ...
        public double x_up_move(double previous_x)
        {
        ...
        public double x_down_move(double previous_x)
        {
            double x_pitch_size = Convert.ToDouble(pitch_x.Value);
        ...
        private async void shutter_sequence_oneshot()
        {
            settings1.OpenTime = (Convert.ToInt16(expo_time_1.Value) * 1000);
            settings1.ClosedTime = 1000;
            settings1.NumberOfCycles = 0;

            settings2.OpenTime = (Convert.ToInt16(expo_time_2.Value) * 1000);
            settings2.ClosedTime = 1000;
            settings2.NumberOfCycles = 0;

            settings3.OpenTime = (Convert.ToInt16(expo_time_3.Value) * 1000);
            settings3.ClosedTime = 1000;
            settings3.NumberOfCycles = 0;



            device1.SetCycleParams(settings1);
            device2.SetCycleParams(settings2);
            device3.SetCycleParams(settings3);
            ...
            //var shutter2 = Task.Factory.ContinueWhenAny(shutter1, shutter2_oneshot);
        ...
        {
            int x_count_1 = Convert.ToUInt16(x_array.Value) * 2;
            int y_count_1 = Convert.ToUInt16(y_array.Value) * 2;
            ...
            int delay = Convert.ToUInt16(delay_time.Value * 1000);
            ...
            int end_count = x_count_1;

            progressBar1.Maximum = x_count_1 - 1;
            ...
            /*
            else if (z_limt > 50)//z 센서 거리 초과
            {
                //var comm = move_axis("0", x_stage_point, y_stage_point);
                //send2controller(comm);
                textBox3.Text = ("OVER Z-AXIS LIMIT");
            }
            */
            ...
            {
                for (int i = 0; i < x_count_1; i++) //y가 1씩 증가할 때
                {
                    var task_1 = Task.Run(() =>
                    {
                        while (true)
                        {
                            var comm_posi = posi_check_robot("0");
                            xy_position_compare(comm_posi);

                            double x_point_db = CustomRound(RoundType.Truncate, x_stage_point, 3);
                            double y_point_db = CustomRound(RoundType.Truncate, y_stage_point, 3);

                            if (x_complete == Convert.ToDecimal(x_point_db)
                                && y_complete == Convert.ToDecimal(y_point_db)) // y축 좌표 비교 이동 완료
                            {
                            ...
                            {
                                var comm_start = move_axis_xy("0", x_stage_point, y_stage_point);
                                send_function(comm_start);
                            }
                        }
                    });
                    await task_1;
                    ...
                    if (i % 2 == 0) //y카운트 짝수일 때
                    {
                        ...
                        for (int m = 1; m < y_count_1; m++)
                        {
                            y_stage_point = y_up_move(y_stage_point);
                            var comm_even_x = move_axis_all_y("0", x_stage_point, y_stage_point);
                            ...
                            if (m == y_count_1 - 1)
                            {
                                ...
                                x_stage_point = x_up_move(x_stage_point);
                                var comm_even_x_up = move_axis_all_x("0", x_stage_point, y_stage_point);
                                send_function(comm_even_x_up);
                    ...
                    else    //홀수
                    {
                        ...
                        for (int r = 1; r < y_count_1; r++) //홀수
                        {
                            y_stage_point = y_down_move(y_stage_point);
                            var comm_odd_y = move_axis_all_y("0", x_stage_point, y_stage_point);
                            send_function(comm_odd_y);
                            ...
                            if (r == y_count_1 - 1)
                            {
                                ...
                                x_stage_point = x_up_move(x_stage_point);
                                var comm_odd_x_up = move_axis_all_x("0", x_stage_point, y_stage_point);
                                send_function(comm_odd_x_up);
                ...
                var comm_all_end = move_axis_xy("0", x_start, y_start);
            ...
            {
                textBox3.Text = "Start Setup position";
            }
        }
        ...
        {
            textBox3.ResetText();
            textBox3.AppendText("shutter open");
        }
        public void shutter_close_print()
        {
            textBox3.ResetText();
            textBox3.AppendText("shutter close");
        ...
        {
            textBox3.ResetText();
            textBox3.AppendText("Z-Axis down ok");

        }
        public void finish_print()
        {
            textBox3.ResetText();
            textBox3.AppendText("job finish");
        }
        ...
        public void ShutterList(ShutterType shutterType)//, double value, int digit = 1)
        {
            //double dReturn = 0;
            ...
            switch (shutterType)
            {
                case ShutterType.One:
                    //dReturn = Math.Ceiling(value * digitCal) / digitCal;
                    //ex) device1 = red, device1 = green, device1 = blue 
                    device1 = device2;
                    device2 = device3;
                    device3 = device1;
